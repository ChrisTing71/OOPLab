...
std::shared_ptr<SDL_Surface> LoadSurface(const std::string &filepath) {
  auto surface =
      std::shared_ptr<SDL_Surface>(IMG_Load(filepath.c_str()), SDL_FreeSurface);
...
void Image::Draw(const Core::Matrices &data) {
  ...
  GLint tintLoc = glGetUniformLocation(s_Program->GetId(), "tintColor");
  glUniform4f(tintLoc, m_TintColor.r, m_TintColor.g, m_TintColor.b,
              m_TintColor.a);

  // Set fill progress uniform
  GLint fillLoc = glGetUniformLocation(s_Program->GetId(), "fillProgress");
  glUniform1f(fillLoc, m_FillProgress);

  // Set show fill progress uniform
  GLint showFillLoc =
      glGetUniformLocation(s_Program->GetId(), "showFillProgress");
  glUniform1i(showFillLoc, m_ShowFillProgress ? 1 : 0);
