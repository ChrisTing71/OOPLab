...
void Renderer::Update() {
  ...
  for (const auto &child : m_Children) {
    stack.push_back(StackInfo{child, m_Translation});
  }
  ...
  while (!stack.empty()) {
    ...
    for (const auto &child : curr.m_GameObject->GetChildren()) {
      stack.push_back(StackInfo{
          child,
          curr.m_ParentTranslation +
              curr.m_GameObject->GetTransform().translation,
      });
    }
  }
  ...
  while (!renderQueue.empty()) {
    ...
    const glm::vec2 originalTranslation =
        curr.m_GameObject->m_Transform.translation;
    curr.m_GameObject->m_Transform.translation =
        originalTranslation + curr.m_ParentTranslation;
    curr.m_GameObject->Draw();
    curr.m_GameObject->m_Transform.translation = originalTranslation;
  }
}
} // namespace Util
