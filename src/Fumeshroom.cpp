...
bool Fumeshroom::StartAttack(const std::vector<std::string> &attackFramePaths,
                             const std::size_t frameIntervalMs) {
  if (attackFramePaths.empty() || IsAttacking()) {
    return false;
  }

  m_AttackAnimation = std::make_shared<Util::Animation>(
      attackFramePaths, true, frameIntervalMs, false, 0);
...
bool Fumeshroom::UpdateAttackStateAndCheckShoot() {
  if (!IsAttacking()) {
    return false;
  }
...
void Fumeshroom::ApplyScaleForCurrentDrawable(const float targetHeightPx) {
